...
export class StatisticalComponent {
  ...
  handleNavbarClick() {
    ...
    if (classNavRight) {
      classNavRight.className = classNavRight.className === "col c-12 m-12 l-12" ? "col c-12 m-9 l-9" : "col c-12 m-12 l-12";
      if (classNavLeft) {
  ...
  updateChartDataPie() {
    ...
    for (let key in this.dataRevenuePie) {
      if (this.dataRevenuePie.hasOwnProperty(key)) {
        let pair = { name: key === 'waiting' ? "Đơn hàng chờ duyệt" : (key === 'transporting' ? "Đang vận chuyển" : (key === "destroy" ? "Đơn hàng bị hủy" : (key === "delivered" ? "Đơn hàng đã giao" : key))), 
